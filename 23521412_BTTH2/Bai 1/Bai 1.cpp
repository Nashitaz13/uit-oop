...
int main()
{
    ...
    C.SetTungDo(4); // Thiết lập tung độ mới cho điểm C
    C.Xuat(); // Xuất thông tin điểm C (5, 4)
    C.TinhTien(1, -1); // Di chuyển điểm C 1 đơn vị sang phải và 1 đơn vị xuống dưới
    C.Xuat(); // Xuất thông tin điểm C (6, 3)
    cout << C.GetHoanhDo() << endl; // In ra hoanh do diem C
    cout << C.GetTungDo() << endl; // In ra tung do diem C
    cout << endl;
    return 0;
}
